...
//
// Lifted verbatim from hypxrvoice's HUD subprocess (src/hud/Egl.cpp @ 200a80e),
// itself adapted from HypXRland's CXRGraphics: no blit program, no external-OES
// texture, no Aquamarine coupling — one context to upload panel textures and to
// satisfy the OpenXR EGL binding.
//
...
// SYNCHRONIZE. So our context must be CURRENT across those calls. hypxrhud keeps
// it current on the single (main) thread for the whole session — see Session.cpp.
namespace hud {
class CEgl {
  public:
    CEgl()  = default;
