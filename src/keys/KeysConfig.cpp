...
bool parseKeysConfig(const std::string& text, SKeysConfig& out,
    ...
    auto setBool = [&](bool& target, const std::string& value) {
        if (!asBool(value, target))
    ...
    while (std::getline(input, line)) {
        ...
        if (key == "rules") setString(out.rules, value);
        else if (key == "model") setString(out.model, value);
        else if (key == "layout") setString(out.layout, value);
        else if (key == "variant") setString(out.variant, value);
        else if (key == "options") setString(out.options, value);
        else if (key == "slot") setString(out.slot, value);
        ...
        else if (key == "privacy_hold_ms") setInt(out.privacyHoldMs, value);
        else if (key == "mods_only") setBool(out.modsOnly, value);
        else if (key == "opacity") {
            if (!asDouble(value, out.opacity))
                error("expected a number");
        } else
            warning("unknown key 'keys." + key + "' (ignored)");
    }

    if (out.layout.empty()) errors.push_back("keys.layout must not be empty");
    if (out.slot.empty()) errors.push_back("keys.slot must not be empty");
    if (out.history < 1 || out.history > 8) errors.push_back("keys.history must be in [1,8]");
    if (out.coalesceMs < 0 || out.coalesceMs > 10000) errors.push_back("keys.coalesce_ms must be in [0,10000]");
    if (out.riseMs < 0 || out.privacyHoldMs < 0)
        errors.push_back("keys timing values must be non-negative");
    if (out.privacyHoldMs < std::max(250, out.riseMs + 100))
        errors.push_back("keys.privacy_hold_ms must cover rise_ms plus the disclosure dwell");
    if (out.opacity < 0.2 || out.opacity > 1.0) errors.push_back("keys.opacity must be in [0.2,1]");
    return errors.empty();
}
...
bool loadKeysConfigFile(const std::string& path, SKeysConfig& out,
    ...
    return parseKeysConfig(contents.str(), out, errors, warnings);
}

std::string defaultKeysConfigPath() {
    if (const char* xdg = std::getenv("XDG_CONFIG_HOME"); xdg && *xdg)
        return std::string(xdg) + "/hypxrhud/keys.toml";
    if (const char* home = std::getenv("HOME"); home && *home)
        return std::string(home) + "/.config/hypxrhud/keys.toml";
    return "keys.toml";
}
