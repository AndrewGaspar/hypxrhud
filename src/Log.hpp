...
// Minimal stderr logger, shared with the hypxrpaper / hypxrvoice sibling projects.
// hypxrhud is a standalone daemon, so we log straight to stderr (line-flushed)
// rather than through any compositor logger. A systemd user unit captures this
// into the journal.
namespace Log {
    enum eLevel {
        TRACE,
